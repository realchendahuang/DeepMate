...
(`motion/react`) — uses the tokens in `src/shared/lib/motion.ts`:
`MOTION_DURATION.fast` (140ms) for list reflow and page exits,
`MOTION_DURATION.base` (180ms) for page and section entrances, plus the
shared `MOTION_EASE` bezier curves and the `enterTransition` /
`pageVariants` presets. Durations and easings are design tokens: never inline
a raw number in a component.
...
changes; section entrances use the shared `enterTransition`.

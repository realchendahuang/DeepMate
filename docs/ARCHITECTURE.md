...
DeepMate stores no secrets today: an `api_key_env` is the *name* of the
environment variable the harness reads, never the key itself. If a future
feature needs to hold a DeepMate-owned secret (private registry credentials,
say), it must use the operating system's credential store through a
cross-platform Rust keyring abstraction rather than a file here.
...
The crate is deliberately small; it holds types and file I/O, not managers.
Its modules:

```text
model.rs     domain types (Detection, RuntimeStatus, Profile, Provider, Model,
             Plugin, MarketEntry, DoctorReport, ...)
data.rs      the data layout, TOML configuration and JSONL action history
snapshot.rs  portable harness inventories (capture, store, validate)
backup.rs    portable DeepMate-settings backups
update.rs    release comparison, asset selection, checksum verification
fsutil.rs    atomic writes and quarantining of unreadable files
error.rs     the CoreError classifications (and their stable codes)
```

Orchestration lives in the `deepseek-harness` service crate (runtime
lifecycle, plugin and market operations, snapshots, doctor), which is what
the CLI and the desktop commands call.
...
the two flows stay consistent. The disabled row's remove action drops the
